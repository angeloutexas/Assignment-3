...
def prepare_game():
    ...
    if len(sys.argv) == 2:
        ...
        if secret_word.isdigit():
            seed = int(secret_word)
            random.seed(seed)
            secret_words = []
            with open("secret_words.txt", "r", encoding= "ascii") as secret_words_file:
                for line in secret_words_file:
                    secret_words.append(line.strip())
            secret_word = random.choice(secret_words)
           
        elif secret_word.islower() and secret_word.isalpha() and len(secret_word) == 5:
            pass
        else:
            secret_word = "hello"
            raise ValueError("Invalid input")
    else:
        raise ValueError("Only one argument needed")
...
def is_valid_guess(guess, valid_guesses):
    ...
    with open("valid_guesses.txt", "r", encoding="ascii") as unlisted_valid_guesses:
        valid_guesses.extend(word.rstrip() for word in unlisted_valid_guesses)
